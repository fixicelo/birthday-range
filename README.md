...
- `.year(year: number | string)`: Restricts birthdays to a specific year.
- `.age(age: number | string, options?: { asOfDate?: string | Temporal.PlainDate })`: Calculates the date range for a given age as of a reference date (defaults to today).
- `.month(month: number | string)`: Restricts birthdays to a specific month (1-12).
- `.day(day: number | string)`: Restricts birthdays to a specific day of the month (1-31).
